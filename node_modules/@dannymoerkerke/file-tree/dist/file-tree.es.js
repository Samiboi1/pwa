class MaterialLoader extends HTMLElement{static get observedAttributes(){return["size"]}constructor(){super();const a=this.attachShadow({mode:"open"});a.innerHTML=`
      <style>
        :host {
          --loader-color: #008000;
          --line-width: 3px;
    ...
    `,this.svg=this.shadowRoot.querySelector("svg")}attributeChangedCallback(a,b,c){"size"===a&&(this.svg.style.width=`${parseInt(c,10)}px`)}}customElements.get("material-loader")||customElements.define("material-loader",MaterialLoader);class MaterialDialog extends HTMLElement{constructor(){super();const a=this.attachShadow({mode:"open"});a.innerHTML=`
            <style>
                :host {
  ...
  `,this.button=this.shadowRoot.querySelector("button"),this.label=this.shadowRoot.querySelector("#label"),this.ripple=this.shadowRoot.querySelector(".ripple")}connectedCallback(){this.hasAttribute("label")?this.label.textContent=this.getAttribute("label"):this.label.style.display="none",this.button.addEventListener("click",()=>{this.active=!0}),this.ripple.addEventListener("animationend",()=>{this.active=!1})}attributeChangedCallback(a){"label"===a&&(this.hasAttribute("label")?this.label.textContent=this.getAttribute("label"):this.label.style.display="none")}get disabled(){return this.hasAttribute("disabled")}set disabled(a){this.button.disabled=a,a?this.setAttribute("disabled",""):this.removeAttribute("disabled")}get active(){return this.hasAttribute("active")}set active(a){a?this.setAttribute("active",""):this.removeAttribute("active")}}customElements.get("material-button")||customElements.define("material-button",MaterialButton);let handler;const onOutsideClick=(a,b)=>{handler=c=>{const[d]=c.composedPath();d!==a&&10!==d.compareDocumentPosition(a)&&b()},document.body.addEventListener("click",handler)},removeOutsideClick=()=>{handler&&document.body.removeEventListener("click",handler)};class ContextMenu extends HTMLElement{static get observedAttributes(){return[]}constructor(){super();const a=this.attachShadow({mode:"open"});a.innerHTML=`
      ...
      <ul part="menu"></ul>      
    `}connectedCallback(){this.menu=this.shadowRoot.querySelector("ul"),this.hide(),onOutsideClick(this.menu,()=>this.hide())}set options(a){this.menu.innerHTML="",a.forEach(({label:a,callback:b,disabled:c})=>{const d=document.createElement("li"),e=a.toLowerCase().replace(" ","-").trim();c&&d.setAttribute("disabled",""),d.dataset.action=e,d.innerText=a,d.addEventListener("click",async a=>{await b(a),this.hide()}),this.menu.appendChild(d)}),this.style.opacity="0",this.removeAttribute("hidden"),this.menuHeight=this.offsetHeight,this.setAttribute("hidden",""),this.style.opacity="1"}show({x:a=0,y:b=0}){b+this.menuHeight>document.documentElement.clientHeight&&(b-=this.menuHeight),this.style.top=`${b}px`,this.style.left=`${a}px`,this.removeAttribute("hidden")}hide(){this.setAttribute("hidden","")}}customElements.define("context-menu",ContextMenu);const workerBlob=new Blob([//language=JavaScript
`
  ...
  `]),worker=URL.createObjectURL(workerBlob);class FileTree extends HTMLElement{constructor(){super();const a=this.attachShadow({mode:"open"});a.innerHTML=`
      <style>
        :host {
          --font-family: Arial;
          --font-color: #000000;
